...
macro table(kclass, **names)
	class {{kclass.id}}
		...
		JSON.mapping(
			"id": Int32,{% for key, value in names %}
			"{{key}}" : {{value}}?,{% end %}
			"created_at": Time?,
			"updated_at": Time?
		)

		# def initialize({% for key, value in names %}
		# 	@{{key.id}} : {{value.id}},{% end %})
		# end
		...
		def self.rand_insert
			sql =<<-END
			insert into #{@@table_name} set {% for key, value in names %}{{key}} = '#{VALUES["{{value}}"]}',{% end %}
			END
			exec sql.rchop(',')
		end
		...
		def self.find(id : Int32)
			query("select * from #{@@table_name} where id = #{id}")[0]
		end
